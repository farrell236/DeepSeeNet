...
def RPD_Model_1(n_classes=1, input_shape=(224, 224, 3), weights='imagenet',
    ...
    return tf.keras.Sequential([
        ...
        tf.keras.layers.Dropout(dropout),
        tf.keras.layers.Dense(n_classes, activation='softmax', name='cfp_predictions'),
    ], name=name)
...
class RPD_Model_2(tf.keras.Model):
    def __init__(self, n_classes=1, input_shape=(224, 224, 3), weights='imagenet',
        ...
        self.combined_predictions = tf.keras.layers.Dense(n_classes, activation='softmax')

    def call(self, faf_input, cfp_input):

        faf = self.faf_branch(faf_input)
        cfp = self.cfp_branch(cfp_input)
